...
from rs_data import SkillType
from rs_data import (PCA, TSNE, UMAP)
from rs_data.database.rs_processing import Leaderboards
from .dataframes import get_dataframe as boss_specific_dataframe
...
class MinigameExperiments:
    ...
    def __init__(self, N_JOBS = -1):
        ...
        self.activities = ['Targetted bossing']
    ...
    def get_data(self):
        df, formatter = boss_specific_dataframe(primary_activity='Barrows Chests',limit=self.user_limit, aggregate=True, skill_type=self.skill_type)

        max_components = sum([
            len(formatter.agg_skills),
            len(formatter.agg_minigames),
            len(formatter.extra_features),
            len(formatter.live_skills),
            len(formatter.live_minigames),
        ])
    ...
    def run_activity(self, activity):
        ...
        print(activity)
        X, y, preprocessor = self.get_data()
